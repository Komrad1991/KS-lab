...
export class HotkeyManager {
  ...
  private initListener(): void {
   ...
   registerShortcut(keys: string, commandId: string, preventDefault = false): void {
     ...
     if (trimmed.includes(' ')) {
       ...
       this.sequences = this.sequences.filter(s => s.sequence.join(' ') !== normalizedSeq)
  ...
  unregisterShortcut(keys: string, commandId?: string): void {
    ...
    if (!commandId) {
      ...
      // remove any matching sequences by commandId (best-effort)
      this.sequences = this.sequences.filter((s) => s.commandId !== commandId)
      return
    }
