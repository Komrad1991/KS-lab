...
- При поиске команды, которые есть в истории, получают бонус +1000 к fuzzy-скорингу, чтобы оказываться выше
- Поддержка «недавних страниц» через `getRecentCommands()` и автоматическую регистрацию навигационных команд
...
// Простая команда
service.registerCommand('hello', 'Say Hello', 'Shows hello', undefined, () => alert('Hi'))

// Команда с аргументами
service.registerCommand('search', 'Search', 'Search the web', undefined, (args) => {
...
service.registerCommand('longtask', 'Long task', 'Runs long task', undefined, async (signal) => {
  for (let i = 0; i < 10; i++) {
